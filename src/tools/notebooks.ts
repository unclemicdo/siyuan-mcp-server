...
import type { NotebooksData, Notebook } from "../types.js";
...
- 用途：查看所有笔记本并获取 ID，以便后续操作`,
    ...
    async () => {
      try {
        const data = await siyuanPost<NotebooksData>(
...
- 创建名为"工作日志"的笔记本`,
    ...
    async ({ name }) => {
      try {
        const data = await siyuanPost<{ notebook: Notebook }>(
