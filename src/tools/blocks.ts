...
const DataTypeSchema = z
  ...
  .default("markdown")
  .describe(
    "块内容的格式：'markdown'（Kramdown/Markdown 格式，推荐）或 'dom'（HTML DOM 格式）"
  );
...
- data="- 列表项\\n- 另一项" 插入列表块`,
    ...
    async ({ data, dataType, nextID, previousID, parentID }) => {
      try {
        const body: Record<string, unknown> = { data, dataType };
        if (nextID) body.nextID = nextID;
        if (previousID) body.previousID = previousID;
        if (parentID) body.parentID = parentID;

        const result = await siyuanPost<BlockResult[]>(
...
- 读取一个标题的文本：id=<块ID>`,
    ...
    async ({ id }) => {
      try {
        const data = await siyuanPost<KramdownData>(
...
- 列出文档的所有顶级块：id=<文档ID>`,
    ...
    async ({ id }) => {
      try {
        const data = await siyuanPost<ChildBlock[]>(
